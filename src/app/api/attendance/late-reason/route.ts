...
export async function POST(req: Request) {
    try {
        ...
        if (!shift.isLate) {
            return NextResponse.json({ error: "Tu entrada de hoy no está marcada como tarde." }, { status: 400 });
        }

        const updated = await prisma.shift.update({
            where: { id: shift.id },
            data: { lateReason: text, lateReasonAt: new Date() },
        });
        ...
        console.error("[POST /api/attendance/late-reason] Error:", err);
